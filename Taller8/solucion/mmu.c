...
paddr_t mmu_init_kernel_dir(void) {
  ...
  for (int i = 0; i < 1024; i++) {
    ...
    // No hace falta multiplicarlo por 4096=2^12 porque solo tiene los bits 31:12 de la dirección.
    kpt[i].page = i; 
  }
...
void mmu_map_page(uint32_t cr3, vaddr_t virt, paddr_t phy, uint32_t attrs) {
  // Ver slide 16 de la diapo del taller.
  ...
  if ((pd[pd_index].attrs & MMU_P) == 0) {
    ...
    // Por defecto ponemos estos atributos en el Page-Directory Entry
    // para que las Page-Table Entry tengal la decisión final sobre qué
    // permisos hay.
    pd[pd_index].attrs = attrs | MMU_P | MMU_U | MMU_W;
    pd[pd_index].pt = pt >> 12;
  }

  // Hacemos que la página en el índice indicado en la memoria virtual, apunte a la dirección
  // física deseada.
...
bool page_fault_handler(vaddr_t virt) {
  ...
  if (ON_DEMAND_MEM_START_VIRTUAL <= virt && virt <= ON_DEMAND_MEM_END_VIRTUAL) {
    mmu_map_page(rcr3(), virt & 0xfffff000, ON_DEMAND_MEM_START_PHYSICAL, MMU_P | MMU_W | MMU_U);
    return true;
  }
  return false;
}
